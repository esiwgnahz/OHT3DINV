...
for i = 1:1:numel(P)
    test_list = [test_list; ...
        (2*pi)/P(i) 1 V*pi/P(i) 2; ...
        (2*pi)/P(i) 1 V*pi/P(i) 3; ...
        (2*pi)/P(i) 1 V*pi/P(i) 4; ...
        (2*pi)/P(i) 1 V*pi/P(i) 5; ...
        (2*pi)/P(i) 2 V*pi/P(i) 3; ...
        (2*pi)/P(i) 2 V*pi/P(i) 4; ...
        (2*pi)/P(i) 2 V*pi/P(i) 5; ...
        (2*pi)/P(i) 3 V*pi/P(i) 4; ...
        (2*pi)/P(i) 3 V*pi/P(i) 5; ...
        (2*pi)/P(i) 4 V*pi/P(i) 5; ...
    ];
end
...
for i = 1:1:num_totalstims
    ...
    subplot(1,2,1);
    pc3 = pcolor(grid{1},grid{2},amp_field);
    set(pc3,'LineStyle','none')
    title(['Pumping test ', num2str(i), ', ln(Amplitude) field'])
    axis equal
    axis([xmin xmax ymin ymax])
    subplot(1,2,2);
    pc4 = pcolor(grid{1},grid{2},phase_field);
...
for i = 1:1:numobs
    ...
    subplot(1,2,1)
    pc1 = pcolor(grid{1},grid{2},ampK_sensmap);
    set(pc1,'LineStyle','none')
    ...
    subplot(1,2,2)
    pc2 = pcolor(grid{1},grid{2},sensmapSs);
    set(pc2,'LineStyle','none')
...
hold on
pc1 = pcolor(grid{1},grid{2},reshape(params_best(1:num_cells),num_y,num_x));
plot(well_locs(:,1),well_locs(:,2),'ok')
set(pc1,'LineStyle','none','FaceColor','interp')
hold off
...
subplot(1,2,2)
hold on
pc1 = pcolor(grid{1},grid{2},reshape(params_best((num_cells+1):(2*num_cells)),num_y,num_x));
plot(well_locs(:,1),well_locs(:,2),'ok')
set(pc1,'LineStyle','none','FaceColor','interp')
hold off
...
subplot(1,2,1)
hold on
pc2 = pcolor(grid{1},grid{2},reshape(params_true(1:num_cells),num_y,num_x));
plot(well_locs(:,1),well_locs(:,2),'ok')
set(pc2,'LineStyle','none','FaceColor','interp')
hold off
title('True ln(K[m/s])')
xlabel('x (m)')
ylabel('y (m)')
...
subplot(1,2,2)
hold on
pc2 = pcolor(grid{1},grid{2},reshape(params_true((num_cells+1):(2*num_cells)),num_y,num_x));
plot(well_locs(:,1),well_locs(:,2),'ok')
set(pc2,'LineStyle','none','FaceColor','interp')
hold off
...
save('inverse_test_2D_run1_out.mat')
